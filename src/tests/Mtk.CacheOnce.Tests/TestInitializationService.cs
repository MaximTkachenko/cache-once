...
namespace Mtk.CacheOnce.Tests
{
    public class TestInitializationService
    {
        ...
        public (int Value, TimeSpan Ttl) InitWithTtl()
        {
            Interlocked.Increment(ref CountOfInitializations);
            Thread.Sleep(100);
            return (Thread.CurrentThread.ManagedThreadId, TimeSpan.FromHours(1));
        }
        ...
        public async Task<(int Value, TimeSpan Ttl)> InitWithTtlAsync()
        {
            Interlocked.Increment(ref CountOfInitializations);
            await Task.Delay(100);
            return (Thread.CurrentThread.ManagedThreadId, TimeSpan.FromHours(1));
        ...
        public async Task<(int Value, TimeSpan Ttl)> FailedWithTtAsync()
        {
            Interlocked.Increment(ref CountOfInitializations);
            await Task.Delay(100);
            throw new Exception("doesn't work");
        ...
        public async Task<(int Value, TimeSpan Ttl)> UnstablepWithRetryWithTtlAsync(int successAfter)
        {
            int retryCount = 5;
            int retryIteration = 0;
            var toWait = TimeSpan.FromMilliseconds(20);
            ...
            while (true)
            {
                try
                {
                    return (await UnstableHttpAsync(successAfter), TimeSpan.FromHours(1));
                }
                catch
                {
                    retryIteration++;
